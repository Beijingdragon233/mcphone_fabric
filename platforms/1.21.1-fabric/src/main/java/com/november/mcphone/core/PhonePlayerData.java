...
import com.november.mcphone.core.net.SyncPhoneTerminalPacket;
 ...
 * 各写各的 {@code player.getAttachedOrCreate(ModAttachments.XXX)}，那二十多处在两支之间
...
public final class PhonePlayerData {
    ...
    public WallpaperData wallpaper() {
        return player.getAttachedOrCreate(ModAttachments.WALLPAPER);
    }

    public void setWallpaper(WallpaperData value) {
        player.setAttached(ModAttachments.WALLPAPER, value);
    }

    // ---- 聊天已读进度 ----

    public ChatReadState chatRead() {
        return player.getAttachedOrCreate(ModAttachments.CHAT_READ);
    }

    public void setChatRead(ChatReadState value) {
        player.setAttached(ModAttachments.CHAT_READ, value);
    }
    ...
    public void setDisc(DiscState value) {
        player.setAttached(ModAttachments.DISC, value);
    }
    ...
    public void setNotes(NoteList value) {
        player.setAttached(ModAttachments.NOTES, value);
    }

    // ---- 已购 App ----

    public PurchasedApps purchasedApps() {
        return player.getAttachedOrCreate(ModAttachments.PURCHASED_APPS);
    }

    public void setPurchasedApps(PurchasedApps value) {
        player.setAttached(ModAttachments.PURCHASED_APPS, value);
    }
    ...
    public ScriptKv scriptKv() {
        return player.getAttachedOrCreate(ModAttachments.SCRIPT_KV);
    }

    public void setScriptKv(ScriptKv value) {
        player.setAttached(ModAttachments.SCRIPT_KV, value);
    }
    ...
    public ScriptGuards scriptGuards() {
        return player.getAttachedOrCreate(ModAttachments.SCRIPT_GUARDS);
    }
    ...
    public ItemStack terminal() {
        return player.getAttachedOrCreate(ModAttachments.PHONE_TERMINAL);
    }

    /**
     * NeoForge 版这一步之后附件的 {@code sync()} 会自动推给客户端；Fabric 1.21.1
     * 的附件 API 没有同步，这里手工发 {@link SyncPhoneTerminalPacket}。
     * 服务端才有义务发：客户端写附件只发生在收到同步包时（见下），那份写入
     * 不能再发回去，不然就是死循环 —— 用 {@code server} 判别方向。
     */
    public void setTerminal(ItemStack value) {
        player.setAttached(ModAttachments.PHONE_TERMINAL, value);
        if (player.getServer() != null) {
            MCphoneNetwork.sendToPlayer((ServerPlayer) player,
                    new SyncPhoneTerminalPacket(value));
        }
    }
}
