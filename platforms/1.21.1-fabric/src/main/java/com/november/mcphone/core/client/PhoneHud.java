 ...
 * 里，用 {@code registerAbove} 插进原版 HUD 层序列；Fabric 版见 registerLayer
 * 的新层【不带】这道保护。
 *
...
public final class PhoneHud {
    ...
    /**
     * 由 MCphoneClient 挂到 Fabric 的 {@code HudRenderCallback} 上。
     *
     * NeoForge 可以选层（registerAbove 战利品条），Fabric 1.21.1 只有
     * {@code HudRenderCallback} 一条：画在原版整条 HUD 之后 —— 玩法 HUD
     * 之上，但也在 F3 与聊天框之上。顺序比上游晚了两层，影响只在
     * "开着 F3 时手机会盖住调试文字"，不影响任何功能。
     */
    public static void registerLayer() {
        net.fabricmc.fabric.api.client.rendering.v1.HudRenderCallback.EVENT.register(PhoneHud::render);
    }

    //  每 tick 决定挂不挂、要不要唤起鼠标

    /** 由 MCphoneClient 挂到 {@code ClientTickEvents.END_CLIENT_TICK} */
    ...
    private static boolean keyDown(Minecraft mc, net.minecraft.client.KeyMapping mapping) {
        // NeoForge 给 KeyMapping 打了 getKey() 补丁；Fabric 用 accessor 读私有字段
        InputConstants.Key key =
                ((com.november.mcphone.mixin.client.KeyMappingAccessor) (Object) mapping).mcphone$getKey();
