...
public final class AppHotkeyHandler {
    ...
    /**
     * 由 {@code KeyboardHandlerMixin} 从原版 {@code KeyboardHandler.keyPress} 转发。
    ...
    /**
     * 鼠标键那条路。由 {@code MouseHandlerMixin} 从原版 {@code MouseHandler.onPress}
     * 一进门转发（比屏幕分发早），返回 true 表示这一下已被吃掉、原版整个不要处理。
     *
     * 要"取消"而不是事后补：绑在侧键上的 App 打开时不能顺带挥一次手、放一次方块。
     * Fabric 没有 NeoForge 那条可取消的 {@code MouseButton.Pre}，取消语义由 mixin
     * 的 cancellable 注入实现。
     */
    public static boolean onMouseButton(int button, int action) {
        ...
        if (Screens.current(mc) instanceof PhoneScreen phone && phone.captureHotkeyMouse(button)) {
            return true;
        }

        if (Screens.current(mc) != null || mc.player == null || mc.level == null) return false;

        InputConstants.Key key = InputConstants.Type.MOUSE.getOrCreate(button);
        AppHotkeys.Binding pressed = AppHotkeys.Binding.of(key, AppHotkeys.activeModifiers());

        // 原版这一下整个不处理了，自然也没有点击排进队，不必再倒
        return launch(mc, pressed);
    }
