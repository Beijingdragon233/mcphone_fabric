 ...
 *
 * 注册入口：{@link #registerServer()}，由 MCphone.onInitialize 调用。
 */
public final class NetworkHandler {
     ...
     *
     * S2C 的编解码在这里一并登记（服务端发包要用），接收器由
     * {@code MCphoneNetwork.registerToClient} 收进候车室、客户端启动时领走。
     */
    public static void registerServer() {
        ...
        // Fabric 侧追加的两个同步包（NeoForge 靠附件/配置的自动同步，这里手工补）。
        // 处理函数只碰共享类型（ServerConfig 的静态字段 / 附件写入包装在客户端类里），
        // 所以能走 registerToClient，与别的包同一条路
        MCphoneNetwork.registerToClient(
                SyncServerConfigPacket.TYPE,
                SyncServerConfigPacket.STREAM_CODEC,
                NetworkHandler::handleSyncServerConfig
        );
        MCphoneNetwork.registerToClient(
                SyncPhoneTerminalPacket.TYPE,
                SyncPhoneTerminalPacket.STREAM_CODEC,
                NetworkHandler::handleSyncPhoneTerminal
        );
    ...
    /** 客户端收到：服主那份服务端配置推过来了（Fabric 没有自动同步，见 ServerConfig） */
    private static void handleSyncServerConfig(SyncServerConfigPacket packet) {
        ServerConfig.applySync(packet.allowFriendTeleport(), packet.allowChatImages(),
                packet.chatImageMaxKb());
    }

    /** 客户端收到：把终端卡槽的值写到本地玩家附件上（见 PhonePlayerData.setTerminal） */
    private static void handleSyncPhoneTerminal(SyncPhoneTerminalPacket packet) {
        com.november.mcphone.feature.terminal.client.TerminalClientState.apply(packet.terminal());
    }
