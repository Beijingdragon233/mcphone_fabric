...
public final class StoreNetworking {
    ...
    public static void register() {
        MCphoneNetwork.registerToServer(
                RequestPurchasedAppsPacket.class,
                RequestPurchasedAppsPacket::encode,
                RequestPurchasedAppsPacket::decode,
                StoreNetworking::handleRequest
        );

        MCphoneNetwork.registerToServer(
                PurchaseAppPacket.class,
                PurchaseAppPacket::encode,
                PurchaseAppPacket::decode,
                StoreNetworking::handlePurchase
        );

        MCphoneNetwork.registerToClient(
                SyncPurchasedAppsPacket.class,
                SyncPurchasedAppsPacket::encode,
                SyncPurchasedAppsPacket::decode,
    ...
    private static void handleRequest(RequestPurchasedAppsPacket packet, ServerPlayer player) {
    ...
    private static void handlePurchase(PurchaseAppPacket packet, ServerPlayer player) {
        ...
        PurchasedApps owned = ModCapabilities.of(player).purchasedApps();
        ...
        ModCapabilities.of(player).setPurchasedApps(owned.with(appId));
        sync(player);
    ...
    private static void sync(ServerPlayer player) {
        MCphoneNetwork.sendToPlayer(player, new SyncPurchasedAppsPacket(
                ModCapabilities.of(player).purchasedApps()));
    }
    ...
    private static void handleSync(SyncPurchasedAppsPacket packet) {
