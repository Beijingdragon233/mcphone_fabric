 ...
 * <h2>它什么时候不画</h2>
 *
 * F1 隐藏 HUD 时不画。这一条在 1.20.1 上其实是多余的——{@code GameRenderer.render} 写着
 * {@code if (!hideGui || screen != null) gui.render(...)}，hideGui 一真、又没开界面，
 * 整条 overlay 链路根本不派发（相机模式正是靠这一点，见 {@code CameraHandler}）。留着它是
 * 因为那个 {@code || screen != null} ：开着界面时 hideGui 仍会让 gui.render 跑起来。
 * 那一路已经被下面"有界面就不画"挡住了，两道各挡各的，谁先谁后都成立。
 *
...
public final class PhoneHud {
    ...
    /**
     * 这一层的 id。插在原版战利品条之后 —— 玩法 HUD 之上，F3 与聊天框之下。
     *
     * 1.21.1 那边传的是完整的 ResourceLocation；Forge 1.20.1 的
     * {@code registerAbove} 只收路径这一半，命名空间由它拿模组 id 补上，
     * 拼出来的仍是 {@code mcphone:offhand_phone}。
     */
    private static final String OVERLAY_ID = "offhand_phone";
     ...
     *
     * 1.20.1 只有一个 ClientTickEvent，Pre 与 Post 都从这儿进来，不判 phase 会一 tick
     * 触发两次 —— 而本类靠"按下去的那一沿"切开关，触发两次等于一次按键被吃掉一半。
     * END 对应那边的 {@code ClientTickEvent.Post}
     */
    ...
    /**
     * Forge 的 overlay 签名比那边多三个参数：ForgeGui 与画布尺寸都用不上（尺寸每帧
     * 自己从 Window 取，与 syncSize 用的是同一个来源，不会出现两处对不上）。
     * partialTick 那边给的是 DeltaTracker、还要再问它要一个
     * {@code getGameTimeDeltaPartialTick(false)}；这边它本身就是那个 float。
     */
