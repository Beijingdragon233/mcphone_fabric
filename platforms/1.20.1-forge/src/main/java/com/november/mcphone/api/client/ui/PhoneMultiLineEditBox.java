 ...
 * 的笔记于是"打了字，什么都没出现"。200% 吃掉前四行、300% 吃掉前六行。反过来倍数小于 100%
 * 时框比控件大，滚动时正文会溢到框外面去。
 *
 ...
 * 任何钩子能只换掉裁剪——只能整个覆写。【这一份抄的是 1.20.1 的父类】，唯一的改动是那对
 * scissor 换成 {@link GuiUtil#clipped}（它把矩形先过一遍当前的变换矩阵）。
 * 【升 MC 大版本时要照着新的父类再核一遍这一段】——父类改了而这里没跟，表现是背景或
 * 滚动条不对，不会报错。
 *
 * 两支的父类实现核过是同一个形状：1.20.1 的 renderWidget 字节码逐条对下来是
 * {@code visible} 早退 → {@code renderBackground} → {@code enableScissor(x+1, y+1, x+w-1, y+h-1)}
 * → pushPose / translate(0, -scrollAmount, 0) → {@code renderContents} → popPose →
 * {@code disableScissor} → {@code renderDecorations}，与 1.21.1 那一份逐句相同。
 * 父类里 {@code scrollAmount} 是私有字段，两支都要走 protected 的 {@code scrollAmount()}。
 *
...
public final class PhoneMultiLineEditBox extends MultiLineEditBox {
    ...
    /**
     * 滚轮的四参写法 —— <b>这一支上它不是覆写，是个转调</b>。
     *
     * 1.21 把 {@code mouseScrolled} 改成了横竖两个滚动量，1.20.1 只有一个。调用方
     * （{@code NoteEditor}）住在共用层里，两支得能写同一行，所以这边补一个同形的方法
     * 把横向那个丢掉 —— 多行输入框本来就只用得上竖直滚动。
     *
     * 加 {@code @Override} 会编不过：父类上没有这个签名。那正是它必须写在这里、
     * 而不能写进共用层的原因。
     */
