...
public final class ReaderApp extends PhoneApp {
     ...
     *
     * 默认实现是"前置全装了才可用"，而这个 App 一条前置都不声明——照默认走就成了
     * "永远可用"，于是（在只有模组手册这一种书的年代）两个书源模组都没装的整合包里，
     * 主屏上会多一个点开只有空书城的 App。所以自己判。
     *
     * <b>在有本地书源的目标上这一句事实上恒为真</b>（{@code TxtBookSource} 不依赖任何模组）。
     * 还没有本地阅读器的目标上，它照旧取决于玩家装没装 Patchouli 之类。
     * 那是有意的：本地阅读器本身就是这个 App 的功能，玩家没装 Patchouli 也该能看自己的
     * 小说。这一句留着不是白留——它仍然是"能不能出书"这个问题的正确写法，将来本地书源
     * 若也有了不可用的情形（比如目录建不出来），这里不用改。
     */
    @Override
